...
int polisher_main(int argc, char* argv[]) 
{
    ...
    if (numThreads <= 8) {
        ...
        int numGroups = (numThreads <= 16) ? 2 : 4;
        std::vector<int> threadsPerGroup(numGroups, numThreads / numGroups);
        for (int i = 0; i < numThreads % numGroups; ++i) {
            ++threadsPerGroup[i];
        }

        std::vector<std::string> bubblesFiles(numGroups);
        std::vector<std::string> outConsensusFiles(numGroups);
        std::vector<std::thread> threads(numGroups);
        std::vector<std::unique_ptr<BubbleProcessor>> processors;

        for (int i = 0; i < numGroups; ++i) {
            bubblesFiles[i] = bubblesFile;
            size_t dotPos = bubblesFiles[i].find('.');
            bubblesFiles[i].insert(dotPos, "_" + std::string(1, 'a' + i));

            outConsensusFiles[i] = outConsensus;
            dotPos = outConsensusFiles[i].find('.');
            outConsensusFiles[i].insert(dotPos, "_" + std::string(1, 'a' + i));

            processors.push_back(std::make_unique<BubbleProcessor>(scoringMatrix, hopoMatrix, !quiet, enableHopo, threadsPerGroup[i]));
            threads[i] = std::thread(&BubbleProcessor::polishAll, processors[i].get(), bubblesFiles[i], outConsensusFiles[i]);
        }

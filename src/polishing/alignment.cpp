...
AlnScoreType Alignment::addDeletion(unsigned int letterIndex) const
{
    ...
    for (size_t readId = 0; readId < _forwardScores.size(); ++readId)
    {
        ...
        size_t forwardIndex = frontRow * cols;
        size_t reverseIndex = revRow * cols;
        for (size_t col = 0; col < alignedReadsN; col += batchSize, forwardIndex += batchSize, reverseIndex += batchSize)
        {
             // Load elements into SIMD vectors
            __m256i forwardVals = _mm256_load_si256((__m256i*)(forwardScore.data() + forwardIndex));

            __m256i reverseVals = _mm256_loadu_si256((__m256i*)(reverseScore.data() + reverseIndex));
										...
										const std::vector<std::string>& reads) const
{
    ...
    for (size_t readId = 0; readId < reads.size(); ++readId) {
        ...
        const size_t alignedReadsN = cols - cols % batchSize;
        size_t forwardIndex = frontRow * (cols + 1);
        size_t reverseIndex = revRow * (cols + 1) + 1;
        for (size_t col = 0; col < alignedReadsN; col += batchSize, length += batchSize, forwardIndex += batchSize, reverseIndex += batchSize)
        {
            // Load base scores
            __m256i scores = _mm256_loadu_si256((__m256i*)(_subsScores + length));

            __m256i forwardScoreCurrent = _mm256_load_si256((__m256i*)(forwardScore.data() + forwardIndex));

            __m256i forwardScoreNext = _mm256_load_si256((__m256i*)(forwardScore.data() + forwardIndex + 1));

            __m256i reverseScoreNext = _mm256_loadu_si256((__m256i*)(reverseScore.data() + reverseIndex));

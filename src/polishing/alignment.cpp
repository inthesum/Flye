...
//        size_t revRow = reverseScore.nrows() - pos;
//
//        std::vector<AlnScoreType> forwardScoreRow = forwardScore.getRow(frontRow);
//        std::vector<AlnScoreType> reverseScoreRow = reverseScore.getRow(revRow);
//        std::reverse(reverseScoreRow.begin(), reverseScoreRow.end());
//
//        AlnScoreType maxVal = forwardScoreRow[0] + reverseScoreRow[0] + baseScoreWithGap;
//
//        for (size_t i = 0; i < reads[readId].size(); ++i)
//        {
//            char readBase = reads[readId][i];
//            AlnScoreType match = forwardScoreRow[i] + _subsMatrix.getScore(base, readBase);
//            AlnScoreType ins = forwardScoreRow[i+1] + baseScoreWithGap;
//            maxVal = std::max(maxVal, std::max(match, ins) + reverseScoreRow[i + 1]);
//        }
//
//		finalScore += maxVal;
//    }
...
AlnScoreType Alignment::addInsertion(unsigned int pos, char base, const std::vector<std::string>& reads) const {
    ...
    for (size_t readId = 0; readId < reads.size(); ++readId) {
        ...
        for (size_t col = 0; col < alignedReadsN; col += batchSize)
        {
            ...
            __m256i matchScores = _mm256_add_epi64(_mm256_loadu_si256((__m256i*)&forwardScoreRow[col]), scores);

            // Compute insertion scores
            __m256i insertScores = _mm256_add_epi64(_mm256_loadu_si256((__m256i*)&forwardScoreRow[col + 1]), baseScores);

            // Compute max scores
            __m256i maxScores = mm256_max_epi64(matchScores, insertScores);

            // Compute sum
            __m256i sum = _mm256_add_epi64(_mm256_loadu_si256((__m256i*)&reverseScoreRow[col + 1]), maxScores);
        ...
        for (size_t col = alignedReadsN; col < cols; ++col) {
            ...
            AlnScoreType match = forwardScoreRow[col] + _subsMatrix.getScore(base, readBase);
            AlnScoreType ins = forwardScoreRow[col + 1] + baseScoreWithGap;
            maxVal = std::max(maxVal, std::max(match, ins) + reverseScoreRow[col + 1]);
        }
...
//        std::vector<AlnScoreType> forwardScoreRow = forwardScore.getRow(frontRow);
//        std::vector<AlnScoreType> sub(reads[readId].size() + 1);
//        sub[0] = forwardScoreRow[0] + baseScoreWithGap;
//
//        size_t cols = reads[readId].size();
//        const size_t alignedReadsN = cols - cols % batchSize;
//
//        for (size_t col = 0; col < alignedReadsN; col += batchSize)
//        {
//            std::vector<AlnScoreType> scores(batchSize, 0);
//            for(size_t i=0; i<batchSize; i++)
//                scores[i] = _subsMatrix.getScore(base, reads[readId][col + i]);
//
//            // Compute scores for match and insertion
//            __m256i matchScores = _mm256_add_epi64(_mm256_loadu_si256((__m256i*)&forwardScoreRow[col]),
//                                                   _mm256_loadu_si256((__m256i*)&scores));
//            __m256i insertScores = _mm256_add_epi64(_mm256_loadu_si256((__m256i*)&forwardScoreRow[col + 1]),
//                                                    baseScores);
//
//            // Select maximum scores
//            __m256i maxScores = mm256_max_epi64(matchScores, insertScores);
//
//            // Store the maximum scores in the sub vector
//            _mm256_storeu_si256((__m256i*)&sub[col + 1], maxScores);
//        }
//
//        for (size_t col = alignedReadsN; col < cols; ++col) {
//            char readBase = reads[readId][col];
//            AlnScoreType match = forwardScoreRow[col] + _subsMatrix.getScore(base, readBase);
//            AlnScoreType ins = forwardScoreRow[col + 1] + baseScoreWithGap;
//            sub[col + 1] = std::max(match, ins);
//        }
//
//        __m256i maxValues = _mm256_set1_epi64x(std::numeric_limits<AlnScoreType>::lowest());
//        size_t revRow = reverseScore.nrows() - pos;
//        cols = forwardScore.ncols();
//        const size_t alignedForwardScoreN = cols - cols % batchSize;
//        std::vector<AlnScoreType> reverseScoreRow = reverseScore.getRow(revRow);
//        std::reverse(reverseScoreRow.begin(), reverseScoreRow.end());
//
//        // Calculate the maximum score using SIMD
//        for (size_t col = 0; col < alignedForwardScoreN; col += batchSize)
//        {
//            // Calculate sum of sub values and reverse scores
//            __m256i sum = _mm256_add_epi64(_mm256_loadu_si256((__m256i*)&reverseScoreRow[col]),
//                                           _mm256_loadu_si256((__m256i*)&sub[col]));
//
//            // Update maxValues with maximum sum
//            maxValues = mm256_max_epi64(maxValues, sum);
//        }
//
//        // Sum up the final maximum values using horizontal addition
//        AlnScoreType maxVal = std::numeric_limits<AlnScoreType>::lowest();
//        for (size_t col = alignedForwardScoreN; col < cols; ++col)
//        {
//            AlnScoreType sum = sub[col] + reverseScoreRow[col];
//            maxVal = std::max(maxVal, sum);
//        }
//
//        alignas(32) AlnScoreType maxValuesArray[batchSize];

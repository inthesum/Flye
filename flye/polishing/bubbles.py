...
def combine_files(files):
    to_files = []
    for to_file, from_files in files.items():
        to_files.append(to_file)
        print(f"Combining files into: {to_file}")

        try:
            with open(to_file, 'a') as combined_file:
                for from_file in from_files:
                    try:
                        with open(from_file, 'r') as file:
                            data = file.read()
                            if not data:
                                print(f"Warning: {from_file} is empty.")
                            else:
                                print(f"Read {len(data)} bytes from {from_file}")
                            combined_file.write(data)
                            combined_file.flush()  # Ensure data is written to disk
                            print(f"Wrote {len(data)} bytes to {to_file}")
                    ...
                    try:
                        ...
                        print(f"File {from_file} not found. Unable to delete.")
                    except PermissionError:
                        print(f"No permission to delete {from_file}.")
                    except IOError as e:
                        print(f"Error deleting file: {e}")

        except IOError as e:
            print(f"Error opening or writing to {to_file}: {e}")

        print(f"Files combined successfully into {to_file}")
        try:
            file_size = os.path.getsize(to_file)
            print(f"File size: {file_size} bytes")
        except FileNotFoundError:
            print(f"Error: {to_file} not found.")
        except IOError as e:
...
def make_bubbles(alignment_path, contigs_info, contigs_path,
    ...
    to_files = combine_files(get_file_info(bubbles_out, num_proc))

    return coverage_stats, mean_aln_error, to_files
